...
def build_installer_floppy() -> bool:
    ...
    pcntpk_exe = THIRDPARTY / "pcntpk" / "PCNTPK.COM"
    # Verify hard-required files
    required = [kernel_sys, command_com, sys_com, boot_asm, jemmex_exe, oakcdrom, pcntpk_exe]
    for f in required:
        if not f.exists():
    ...
    for src in [jemmex_exe, oakcdrom, pcntpk_exe]:
        result = run(
            ["mcopy", "-i", str(FLOPPY_IMG), str(src), f"::{src.name}"],
            check=False,
        )
    ...
    # Copy FDISK.EXE to floppy so it is available at A:\FDISK.EXE when the
    # installer exits to DOS (the CD may not yet have a drive letter).
    fdisk_src = CDROM_DIR / "FDISK.EXE"
    if not fdisk_src.exists():
        fdisk_src = CDROM_DIR / "FDISK.COM"
    if fdisk_src.exists():
        result = run(
            ["mcopy", "-i", str(FLOPPY_IMG), str(fdisk_src), "::FDISK.EXE"],
            check=False,
        )
        if result.returncode != 0:
            log("WARNING: mcopy failed for FDISK.EXE on floppy — FDISK only available on CD")
    ...
    # Copy INSTALL.EXE onto the floppy so it always runs regardless of whether
    # the CD-ROM driver maps a drive letter.  INSTALL.EXE scans C:-Z: for
    # NOSCD.ID to find the CD after the drive letter extension loads.
    install_exe = SRC_DIR / "install" / "bin" / "INSTALL.EXE"
    if install_exe.exists():
        result = run(
            ["mcopy", "-i", str(FLOPPY_IMG), str(install_exe), "::INSTALL.EXE"],
            check=False,
        )
        if result.returncode != 0:
            log("WARNING: mcopy failed for INSTALL.EXE on floppy — will fall back to D:\\INSTALL.EXE")
            install_on_floppy = False
        else:
            install_on_floppy = True
    else:
        log("  WARNING: INSTALL.EXE not found — installer will not launch")
        install_on_floppy = False
    ...
    install_cmd = "A:\\INSTALL.EXE\r\n" if install_on_floppy else "D:\\INSTALL.EXE\r\n"
...
def stage_iso_root(iso_root: Path) -> bool:
    ...
    required_sys = [
        ...
        THIRDPARTY / "ctmouse" / "CTMOUSE.EXE",
        THIRDPARTY / "pcntpk"  / "PCNTPK.COM",
