...
## [Post-Phase-3 — Installer ISO fully working (QEMU + VBox)] — 2026-04-04

### Fixed

**Installer hanging at DOS prompt (PCNTPK.COM invalid opcode / no PCnet NIC)**

- The installer `AUTOEXEC.BAT` loaded `PCNTPK.COM` via `LH` before launching
  `INSTALL.EXE`.  Without JEMMEX providing UMBs, `LH` loaded conventionally; PCNTPK
  then probed for AMD PCnet hardware.  If no NIC was present the probe hung indefinitely.
  When a PCnet NIC was present, PCNTPK executed an instruction invalid for the QEMU
  i386 CPU default model and crashed with repeated `Invalid Opcode` faults.
- Root cause: `INSTALL.EXE` does not call `DETECT.EXE` during installation (DETECT runs
  on first HDD boot), so PCNTPK was never needed at install time.
- Fix — `build/mkiso.py` `INSTALLER_AUTOEXEC_BAT`: removed `LH A:\PCNTPK.COM INT=0x60`
  from the installer `AUTOEXEC.BAT`.

**Installer crashing with `Invalid Opcode` on second CD boot (warm-reboot IVT corruption)**

- After `auto_partition()` (FDISK on blank disk), `do_reboot()` used INT 19h (bootstrap
  loader warm reset).  INT 19h does not clear the interrupt vector table: `OAKCDROM.SYS`
  had hooked interrupt vectors during the first boot; when CONFIG.SYS reloaded it on the
  second boot, the new instance chained onto its own now-overwritten handlers, causing
  `Invalid Opcode` faults at every subsequent interrupt.
- Fix — `src/install/install.c` `do_reboot()`: replaced INT 19h with a keyboard
  controller reset pulse (port `0x64`, command `0xFE`).  This triggers a full BIOS POST
  which rebuilds the IVT cleanly before the second CD boot.  Added `<conio.h>` for
  `inp()`/`outp()`.

**`FORMAT.EXE` AIRF on blank partition (root cause of all prior VBox/QEMU install failures)**

- FreeDOS `FORMAT.EXE` fails with `Abort/Ignore/Retry/Fail` after writing ~16 sectors
  when run against a completely blank partition (no existing FAT BPB in the partition
  boot record).  Analysis: FORMAT wrote `0xF6` fill bytes to LBA 63–78 then stopped;
  LBA 79 onward remained zero.  The issue reproduces whether JEMMEX is present or not —
  JEMMEX was a red herring for this specific failure.
- Root cause: FORMAT reads an existing BPB to validate disk geometry before re-writing
  the filesystem structures.  With no pre-existing BPB (fully blank partition), the
  geometry derivation fails and FORMAT aborts.
- Fix — `build/mkhdd.py`: added `nosdos-blank.hdd` output — same MBR, same partition
  geometry (64 CYL × 16 HEADS × 63 SPT), same FAT16 format via `mformat`, but no
  NOS-DOS files.  The installer finds C: already partitioned and formatted, skips the
  FDISK/FORMAT path entirely, and copies files directly from the ISO.
- Fix — `build/mkvm.py`: added `make_blank_vmdk()` — converts `nosdos-blank.hdd` to
  `nosdos-blank.vmdk` for VirtualBox installer use.  `run-qemu.sh` updated to use
  `nosdos-blank.hdd` as the installer target disk.
- Fix — `src/install/install.c`: added `/U` flag to `FORMAT.EXE` invocation (belt and
  suspenders: skip verify pass).  Removed `JEMMEX.EXE` and `DOS=HIGH,UMB` from
  `INSTALLER_CONFIG_SYS` in `build/mkiso.py` (installer needs no extended memory).

**Installer workflow (verified working 2026-04-04)**

- QEMU: `run-qemu.sh` → boots ISO with `nosdos-blank.hdd` → installer copies files →
  `run-qemu-hdd.sh` (or `-boot c nosdos-blank.hdd`) → NOS-DOS running, MTCP.CFG
  populated, networking functional.
- VBox: attach `nosdos-blank.vmdk` as IDE HDD + `nosdos.iso` as IDE CD → boot → install
  → detach ISO → reboot from VMDK → same result.
